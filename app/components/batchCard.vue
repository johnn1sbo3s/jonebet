<template>
  <UCard
    class="group max-w-150 border border-zinc-800 bg-zinc-900 hover:cursor-pointer hover:outline hover:outline-teal-500"
    @click="emit('click')"
  >
    <template #header>
      <div class="flex justify-between align-baseline">
        <div class="flex gap-5 font-semibold">
          ...
          <div class="flex gap-3">
            <p :class="metricItem.profit >= 0 ? 'text-teal-500' : 'text-red-500'">
              {{ metricItem.profit > 0 ? '+' : '' }}{{ metricItem.profit }} u
            </p>
        ...
        <i class="i-lucide-chevron-right hidden text-2xl text-teal-500 group-hover:block" />
      </div>
    </template>
    ...
    <template #default>
      <div class="flex w-full gap-8">
        ...
        <div class="w-full">
          <p class="text-center font-semibold text-zinc-300">Intervalo de confiança</p>

          <div class="flex h-[80%] gap-3">
            <p class="self-center font-semibold text-red-500">{{ metricItem.bottom_int_conf }}</p>

            <UDivider label="a" />

            <p class="self-center font-semibold text-teal-500">{{ metricItem.top_int_conf }}</p>
          </div>
        </div>
      </div>
